...
public @interface LLMConfig {
    ...
    /**
     * Maximum number of tokens to generate for a single {@link ChatCompletion}. {@link LLM} may produce fewer tokens
     * than this, but it will never generate more.
     * <p>
     * Either {@code llm.max_tokens} or {@code llm.max_completion_tokens} should be set to a non-zero value (only one of
     * them). Setting both to non-zero values might lead to invalid API responses.
     *
    ...
    /**
     * Parameter that controls the diversity of generated text by reducing the likelihood of repeated sequences. Higher
     * values decrease repetition, but when the value is too high, incoherent sentences might be generated.
     *
     ...
     * with the highest-probability word and adds the next most likely words until their total probability equals or
     * exceeds <i>p</i>. Then it randomly picks from this set.
     * <p>
     * For example, if {@code top_p=0.9}, the model looks at all possible next words, starting with the most likely, and
     * keeps adding words until their combined probability is at least 90%. Any words outside this '90% bubble' are not
     * considered. This method helps strike a balance between picking the single most likely word every time (which can
     * make text feel robotic) and choosing from too wide a range of words (which might make it incoherent). Higher
     * {@code top_p} (e.g., {@code 0.95} or {@code 1.0}) includes more possible words, leading to more varied and
     * creative text, but it can also become less focused. Lower {@code top_p} (e.g., {@code 0.7}) limits the choice to
     * fewer words, making text more predictable and consistent, though sometimes too repetitive.
     *
